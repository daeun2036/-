...
***     
-  논 블로킹이란 이전 작업이 완료될 때까지 대기하지 않고 다음 작업을 수행하는 것을 의미. 반대로 블로킹은 이전 작업이 끝나야만 다음 작업을 수행.
- 파일 시스템 접근, 네트워크를 통한 요청 작업은 I/O의 일종이며 이러한 작업을 할 때 `노드는 논 블로킹 방식으로 처리`   
...
- `노드는 싱글 스레드, 논 블로킹 모델`로, 싱글 스레드가 혼자서 일을 처리하지만 들어온 요청 순서가 아닌 논블로킹 방식으로 이전 작업이 완료될 때까지 대기하지 않고 다음 작업을 수행.


### 5. 서버로서의 노드

***
- 노드 서버 또한 `싱글 스레드`, `논 블로킹` 모델 사용    
